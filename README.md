# Antigravity Checkers Battle Arena 🪐⚡
### Dynamic Programming AI Engine & Cyberpunk Arena

A tournament-grade Checkers (Draughts) game engine and visual arena powered by **Dynamic Programming**, **Minimax with Alpha-Beta Pruning**, **64-bit Zobrist Hashing**, **Retrograde Endgame Analysis**, and the custom **Gravity Shift 180° Inversion** mechanic.
...
1. **High-Performance C++20 DAA Engine** (`cpp/`): Native compiled binary (`antigravity_checkers.exe`) for microsecond-speed search tree evaluations and benchmarking.
2. **Interactive Antigravity Web Arena** (`index.html`, `js/`, `css/`): Real-time interactive UI with Canvas rendering, particle systems, audio synthesizer, and live Dynamic Programming HUD telemetry.

```
+---------------------------------------------------------------------------------------+
|                                    USER INTERFACE                                     |
|  - HTML5 Canvas Glowing Board & Antigravity Particle Physics Engine                   |
|  - Holographic Sci-Fi HUD (Live DP Cache Hit Rate, Search Tree Pruning Telemetry)     |
|  - Mode Selector (PvP, PvC, AI vs AI), Depth Slider, Gravity Shift Button             |
+-------------------------------------------+-------------------------------------------+
                                            |
                                            v
+---------------------------------------------------------------------------------------+
...
|  - Gravity Shift Manager (1x charge per player, reverses piece forward direction)     |
...
| - Bitwise/Vector Move Generators      |   | - Gravity Shift Strategic Utility Check   |
+---------------------------------------+   +---------------------+---------------------+
                                                                  |
                     +--------------------------------------------+---------------------+
                     |                                                                  |
...
$$\text{Hash}(S) = \left( \bigoplus_{r=0}^7 \bigoplus_{c=0}^7 \text{Key}[r][c][\text{piece}] \right) \oplus \text{Key}[\text{turn}] \oplus \text{Key}[\text{gravity}]$$
...
## ⚡ 3. Antigravity Mechanics: "Gravity Shift"

- **Mechanic**: Once per match, each player can trigger a **Gravity Shift**.
- **Effect**: Inverts the gravitational vector $180^\circ$ for all regular pieces:
...
- **AI Evaluation**: The AI agent evaluates whether triggering Gravity Shift produces a net tactical gain (e.g. immediate promotions, trapping opposing pieces) before committing the charge.
...
```bash
# Compile using G++
g++ -std=c++20 -O3 -I cpp/include cpp/src/*.cpp -o antigravity_checkers.exe

# Run interactive CLI & Benchmark
./antigravity_checkers.exe
```
